...
typedef struct {
    ...
    int world;                 // 0=space, 1=arcade, 3=atoll (WORLD_ATOLL, port)
    /* Where STATE_EDIT opens (port extension): edit_level 0 resumes the
     * editor where it was left when edit_world is the world it shows. */
    int edit_world;
    int edit_level;
